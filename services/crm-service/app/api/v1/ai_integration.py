        ...
        "items": [
            {
                "session_id": row.id,
                "lead_id": row.lead_id,
                "lead_name": lead_map[row.lead_id].full_name if row.lead_id in lead_map else None,
...
async def monitor_session_detail(
    ...
    return {
        ...
        "calls": [
            {"id": c.id, "status": c.status, "recording_url": c.recording_url, "created_at": c.created_at}
            for c in calls
        ],
        "conversation": {
...
async def screening_respond(
    ...
    Request body:
        { "answer": "candidate's response to the last question" }

    Response:
        {
            "ai_response": "AI's acknowledgment of their answer",
            "next_question": "next question to ask, or null if done",
            "is_done": bool,
            "sms_message": "...",   // only present when is_done=true
            "email_message": "...", // only present when is_done=true
            "turn_number": int,
        }
    """
    session = (
    ...
    # Load existing answers and questions from session
    ...
    # Build conversation history from questions + answers so far
    ...
    # Store the new answer and the next question in the session
